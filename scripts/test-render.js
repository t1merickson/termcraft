...
function run() {
  ...
  if (updateMode && !fs.existsSync(EXPECTED_DIR))
  ...
  for (const sample of samples) {
    ...
    for (const mode of MODES) {
      const dims = ImageToAnsi.calcDimensions(
        src.width,
        src.height,
        MAX_WIDTH,
        MAX_HEIGHT,
      ...
      console.log(
        `    ${mode.padEnd(14)} ${status.padEnd(4)} → pixel grid ${w}×${h} → ${charW}×${charH} chars → ${outPng.width}×${outPng.height}px${detail}`,
      );
    }

    // Also save source scaled to match half-block output height for comparison
    const halfDims = ImageToAnsi.calcDimensions(
      src.width,
      src.height,
      MAX_WIDTH,
      MAX_HEIGHT,
  ...
  console.log(
    `  Results: ${passed} passed, ${failed} failed, ${newFiles} new\n`,
  );
