...
const spaceWidth = args["space-width"]
  ? Number(args["space-width"])
  : Math.floor(glyphWidth / 2);
const letterGap = args["letter-gap"] ? Number(args["letter-gap"]) : 1;
const fallback = args["fallback"] || "?";
const threshold = args["threshold"] ? Number(args["threshold"]) : 32;
const lumaThreshold = args["luma-threshold"]
  ? Number(args["luma-threshold"])
  : null;
...
if (
  lumaThreshold !== null &&
  (!Number.isFinite(lumaThreshold) || lumaThreshold < 0 || lumaThreshold > 255)
) {
...
if (charset.length > capacity) {
  console.error(
    `Sprite sheet only fits ${capacity} glyphs, but charset has ${charset.length}`,
  );
  process.exit(1);
}
...
const finalSpaceW = Math.max(
  1,
  spaceWidth - pad.leftStripped - pad.rightStripped,
);
