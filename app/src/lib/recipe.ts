...
export function createRecipe<T extends object>(
  toolId: string,
  defaults: T,
): RecipeCodec<T> {
  const encode = (state: T) => {
    ...
    return Object.keys(delta).length
      ? encodeBase64Url(JSON.stringify(delta))
      : "";
  };
...
export function useRecipe<T extends object>(
  ...
  { shareUrl: string; shareCode: string; reset: () => void },
] {
  ...
  const codec = useMemo(
    () => createRecipe(toolId, stableDefaults),
    [toolId, stableDefaults],
  );
  const recipeParam = params.get("r") ?? "";
  const [state, setState] = useState<T>(() => ({
    ...stableDefaults,
    ...codec.decode(recipeParam),
  }));
  ...
  const reset = useCallback(
    () => setState({ ...stableDefaults }),
    [stableDefaults],
  );
  return [
    state,
    patch,
    { shareUrl: codec.toUrl(state), shareCode: codec.toCode(state), reset },
  ];
}
