...
export function rasterize(
  ...
  grid.cells.forEach((row, y) => {
    for (let start = 0; start < row.length;) {
      ...
      while (
        ...
        JSON.stringify(row[end].bg) === JSON.stringify(bg)
      )
        end += 1;
      if (bg) {
        ctx.fillStyle = rgb(bg);
        ctx.fillRect(
          metrics.padding + start * metrics.cellWidth,
          metrics.padding + y * metrics.cellHeight,
  ...
  grid.cells.forEach((row, y) =>
    row.forEach((cell, x) => {
      ...
      ctx.fillText(
        cell.ch,
        metrics.padding + x * metrics.cellWidth,
        metrics.padding +
          y * metrics.cellHeight +
          (metrics.cellHeight - metrics.fontSize) / 2,
      );
    }),
  );
  return canvas;
