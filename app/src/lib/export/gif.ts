...
function bucket(colors: RGB[]): Bucket {
  ...
  for (let c = 0; c < 3; c += 1) {
    ...
    for (const color of colors) {
      lo = Math.min(lo, color[c]);
      hi = Math.max(hi, color[c]);
    }
    if (hi - lo > range) {
      range = hi - lo;
      channel = c;
...
function quantize(colors: RGB[], limit: number): RGB[] {
  ...
  if (counts.size <= limit)
    return [...counts.values()].map((entry) => entry.color);
  ...
  let boxes = [
    bucket(
      [...counts.values()].flatMap(({ color, count }) =>
  ...
  while (boxes.length < limit) {
    ...
    if (candidate.colors.length < 2) {
      boxes.unshift(candidate);
      break;
    }
    candidate.colors.sort(
      (a, b) => a[candidate.channel] - b[candidate.channel],
    );
    const middle = Math.floor(candidate.colors.length / 2);
    boxes.push(
      bucket(candidate.colors.slice(0, middle)),
      bucket(candidate.colors.slice(middle)),
    );
  }
  return boxes.map(
    ({ colors: values }) =>
      [0, 1, 2].map((channel) =>
        Math.round(
          values.reduce((sum, color) => sum + color[channel], 0) /
            values.length,
        ),
      ) as RGB,
  );
...
class Bytes {
  data: number[] = [];
  byte(...values: number[]) {
    this.data.push(...values.map((value) => value & 255));
  }
  word(value: number) {
    this.byte(value, value >> 8);
  }
  ascii(value: string) {
    for (const ch of value) this.byte(ch.charCodeAt(0));
  }
}
...
export async function framesToGif(
  frames: Grid[],
  opts: RasterOptions & { delayMs?: number; loop?: boolean },
): Promise<Blob> {
  ...
  if (width > 65535 || height > 65535)
    throw new Error("GIF dimensions exceed 65535 pixels");
  ...
  images.forEach((image) => {
    const delay = Math.max(
      0,
      Math.min(65535, Math.round((opts.delayMs ?? 100) / 10)),
    );
    out.byte(0x21, 0xf9, 4, hasTransparency ? 1 : 0);
    out.word(delay);
    out.byte(hasTransparency ? transparentIndex : 0, 0);
    out.byte(0x2c);
    out.word(0);
    out.word(0);
    out.word(image.width);
    out.word(image.height);
    out.byte(0);
