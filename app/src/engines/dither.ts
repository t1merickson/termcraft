...
export const PALETTES: Record<string, { label: string; colors: RGB[] }> = {
  ...
  "ansi-16": {
    label: "ANSI 16",
    colors: STANDARD_16.map(
      ({ r, g, b }: { r: number; g: number; b: number }) => [r, g, b] as RGB,
    ),
  },
  "ansi-256": {
    label: "ANSI 256",
    colors: ANSI_PALETTE.map(
      ({ r, g, b }: { r: number; g: number; b: number }) => [r, g, b] as RGB,
    ),
  },
  ...
  "gray-24": {
    label: "ANSI grayscale ramp (24)",
    colors: Array.from(
      { length: 24 },
      (_, i) => [8 + i * 10, 8 + i * 10, 8 + i * 10] as RGB,
    ),
  },
  gameboy: {
    label: "Game Boy",
  ...
  "gameboy-pocket": {
    ...
    colors: ["#000000", "#545454", "#a9a9a9", "#ffffff"].map(hex),
  },
  c64: {
    label: "Commodore 64",
    colors: [
...
function blueNoise(): Uint16Array {
  ...
  scored.forEach((entry, rank) => {
    blueNoiseTile![entry.index] = rank;
  });
...
export function dither(
  ...
  const colors =
    typeof opts.palette === "string"
      ? (PALETTES[opts.palette] ?? PALETTES["mono-1bit"]).colors
      : opts.palette;
  ...
  for (let i = 0, p = 0; i < source.length; i += 4, p += 3) {
    ...
    work[p + 1] = clamp(
      (source[i + 1] - 128) * contrastFactor + 128 + brightness,
    );
    work[p + 2] = clamp(
      (source[i + 2] - 128) * contrastFactor + 128 + brightness,
    );
  }
  ...
  if (kernel) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let step = 0; step < width; step++) {
        ...
        const cp = choice * 3;
        const er = work[wp] - flat[cp],
          eg = work[wp + 1] - flat[cp + 1],
          eb = work[wp + 2] - flat[cp + 2];
        const op = (y * width + x) * 4;
        output[op] = flat[cp];
        output[op + 1] = flat[cp + 1];
        output[op + 2] = flat[cp + 2];
    ...
    let size = 1;
    if (opts.algorithm.startsWith("bayer-")) {
      size = Number(opts.algorithm.slice(6));
      map = BAYER[size as keyof typeof BAYER];
    } else if (opts.algorithm === "halftone") {
      size = 8;
      map = HALFTONE;
    } else if (opts.algorithm === "blue-noise") {
      size = 64;
      map = blueNoise();
    }
    ...
    for (let y = 0; y < height; y++)
      for (let x = 0; x < width; x++) {
        const wp = (y * width + x) * 3;
        const threshold = map
          ? ((map[(y % size) * size + (x % size)] + 0.5) / levels - 0.5) *
            255 *
            strength
          : 0;
        const choice = nearest(
          work[wp] + threshold,
          work[wp + 1] + threshold,
          work[wp + 2] + threshold,
        );
