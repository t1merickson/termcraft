...
export function create(opts) {
  ...
  function renderFrame() {
    ...
    if (autoReduceFactor > 1) {
      options.maxWidth = Math.max(
        20,
        Math.floor(options.maxWidth / autoReduceFactor),
      );
      options.maxHeight = Math.max(
        10,
        Math.floor(options.maxHeight / autoReduceFactor),
      );
    }
  ...
  return {
    ...
    setTargetFps(fps) {
      targetFps = Math.max(1, Math.min(30, fps));
    },
    getAutoReduceFactor() {
      return autoReduceFactor;
    },
  };
}
