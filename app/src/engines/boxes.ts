...
export function stringWidth(value: string): number {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (points[i + 1] === 0xfe0f && !wide(cp)) {
      result += 2;
      continue;
    }
    if (combining(cp) || cp === 0 || cp < 32 || (cp >= 0x7f && cp < 0xa0))
      continue;
    result += wide(cp) ? 2 : 1;
  }
  return result;
...
const alignCell = (
  ...
  if (align === "center")
    return spaces(Math.floor(gap / 2)) + value + spaces(Math.ceil(gap / 2));
...
export function frame(content: string, options: FrameOptions = {}): string {
  ...
  const body = lines.map(
    (line) =>
      `${b.vertical}${spaces(padding)}${alignCell(line, inner, options.align ?? "left")}${spaces(padding)}${b.vertical}`,
  );
  ...
  const top =
    b.topLeft +
    titleText +
    b.horizontal.repeat(Math.max(0, span - stringWidth(titleText))) +
    b.topRight;
  return [
    top,
    ...body,
    b.bottomLeft + b.horizontal.repeat(span) + b.bottomRight,
  ].join("\n");
}
...
export interface TableOptions {
  ...
  padding?: number;
  header?: boolean;
  alignments?: Array<"auto" | "left" | "center" | "right">;
  innerRows?: boolean;
}
export function table(rows: string[][], options: TableOptions = {}): string {
  ...
  const widths = Array.from({ length: columns }, (_, i) =>
    Math.max(1, ...normalized.map((row) => stringWidth(row[i]))),
  );
  const numeric = widths.map((_, i) =>
    normalized
      .slice(options.header ? 1 : 0)
      .filter((r) => r[i].trim())
      .every((r) => Number.isFinite(Number(r[i]))),
  );
  const aligns = widths.map((_, i) =>
    options.alignments?.[i] && options.alignments[i] !== "auto"
      ? (options.alignments[i] as "left" | "center" | "right")
      : numeric[i]
        ? "right"
        : "left",
  );
  const line = (left: string, fill: string, joint: string, right: string) =>
    left + widths.map((w) => fill.repeat(w + padding * 2)).join(joint) + right;
    ...
    (row) =>
      ...
      row
        .map(
          (cell, i) =>
            spaces(padding) +
            alignCell(cell, widths[i], aligns[i]) +
            spaces(padding),
        )
        .join(b.vertical) +
      b.vertical,
  ...
  body.forEach((row, i) => {
    ...
    if (
      i < body.length - 1 &&
      ((options.header && i === 0) || options.innerRows)
    )
...
export function tree(input: string): string {
  ...
  const visit = (
    ...
    output.push(
      root ? node.text : `${prefix}${last ? "└── " : "├── "}${node.text}`,
    );
    const childPrefix = root ? "" : prefix + (last ? "    " : "│   ");
    node.children.forEach((child, i) =>
      visit(child, childPrefix, i === node.children.length - 1, false),
    );
  };
