...
export function renderBraille(
  ...
  for (let row = 0; row < height; row++) {
    let lineAnsi = "",
      lineHtml = "",
      lineText = "",
      lastAnsi: string | null = null;
    let htmlRun = "",
      htmlColor: string | null = null;
    const flushHtml = () => {
      if (!htmlRun) return;
      lineHtml += htmlColor
        ? `<span style="color:${htmlColor}">${htmlRun}</span>`
        : htmlRun;
      htmlRun = "";
    };
    for (let col = 0; col < width; col++) {
      ...
      for (let dy = 0; dy < 4; dy++)
        for (let dx = 0; dx < 2; dx++) {
          ...
          const lum =
            0.299 * maskSource.data[p] +
            0.587 * maskSource.data[p + 1] +
            0.114 * maskSource.data[p + 2];
          const on = a >= 32 && (invert ? lum < threshold : lum >= threshold);
          if (a >= 32) {
            allR += source.data[p];
            allG += source.data[p + 1];
            allB += source.data[p + 2];
            allCount++;
          }
          if (on) {
      ...
      lineText += char;
      let nextAnsi: string | null = null,
        nextHtml: string | null = null;
