...
export function processImage(img, options = {}) {
  ...
  const charsetStr = ramp
    ? ramp.chars
    : CHARSETS[charset] || charset || CHARSETS.standard;
  const { width, height } = calcOutputDimensions(
    img.width,
    img.height,
    maxWidth,
    maxHeight,
    ...
    for (let col = 0; col < width; col++) {
      ...
      if (useShape) {
        ...
        if (directionalContrast) {
          const ext = ShapeVectors.sampleExternalCircles(
            imageData,
            col,
            row,
            cellW,
